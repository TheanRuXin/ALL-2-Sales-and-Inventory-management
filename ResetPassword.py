...
class Reset_Password(ctk.CTkToplevel):
    ...
    def display_ui(self):
        # Password
        self.password_text = ctk.CTkLabel(self, text="New Password",
                                          font=("Inter", 23),
        ...
        self.password_note = ctk.CTkLabel(self,
                                          text="* Password must be at least 8 characters,\n  include one uppercase letter and one number.",
                                          font=("Inter", 14),
                                          bg_color="#FFFFFF", fg_color="#FFFFFF",
                                          text_color="#555555", justify="left")
        self.password_note.place(x=400 / 1920 * self.width, y=210 / 974 * self.height)
    ...
    def reset_password(self):
        ...
        if not self.is_valid_password(password):
            messagebox.showerror("Error", "Password must be at least 8 characters long and include a number and an uppercase letter.", parent=self)
            return
        ...
        try:
            ...
            new_hashed_pw = self.hash_password(password)  # returns string

            cursor.execute("UPDATE users SET password = ? WHERE username = ?", (new_hashed_pw, self.username))
            conn.commit()
            conn.close()
