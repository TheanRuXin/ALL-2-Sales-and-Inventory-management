...
class Setup_Database:
    ...
    def update_item(self, *, product_id, item_name, category, quantity, price, status, registration_date,
                    image_blob=None):
        conn = sqlite3.connect(self.db_file)
        cursor = conn.cursor()
        if image_blob:
...
class ManageProductPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        background_image = ctk.CTkImage(Image.open("assets/Manage_Product.png"),
    ...
    def create_widgets(self):
        ...
        self.product_treeview = ttk.Treeview(frame, columns=("Product_ID", "Name", "Category", "Quantity",
                                                             "Price", "Status", "Register_Date"),
                                             show="headings")
        self.product_treeview.heading("Product_ID", text="Product ID")
        self.product_treeview.column("Product_ID", width=220, anchor="center")
    ...
    def refresh_table(self):
        ...
        for row in self.db.view_items():
            ...
            tags = ("Red",) if int(quantity) < 5 else ()
    ...
    def update_item_handler(self):
        ...
        if all(str(original_values[key]).strip().lower() == str(updated_values[key]).strip().lower() for key in
               original_values):
    ...
    def is_duplicate_name(self, name, exclude_product_id=None):
        for item in self.db.view_items():
            existing_item_id = item[0]
            existing_product_id = item[1]
            existing_name = item[2]
    ...
    def delete_item_handler(self):
        ...
        product_id = self.product_treeview.item(selected_item, "values")[0]  # Using product_id now
    ...
    def search_items(self, query):
        ...
        for row in results:
            ...
            self.product_treeview.insert("", END,
                                         values=(product_id, item_name, category, quantity, price, status,
                                                 register_date),
                                         tags=(stock_colour,))
