...
class ManageProductPage(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        self.category_combobox = ctk.CTkComboBox(self, width=320 / 1536 * self.width,values=self.categories,
                                           height=45 / 864 * self.height, bg_color="#FFFFFF",
                                           fg_color="#ebf9ff", border_color="#0C5481", text_color="black",button_color="#52a0bf",button_hover_color="#0C5481")
        if self.categories:
        ...
        # Price
        self.price_text = ctk.CTkLabel(self, text="Price(RM):",
                                       font=("Inter", 23),
                                       bg_color="#FFFFFF", fg_color="#FFFFFF",
                                       text_color="black")
        ...
        self.update_button = ctk.CTkButton(self, text="Update Product", bg_color="#FFFFFF", fg_color="#0C5481",
                                           text_color="#FFFCFC",
        ...
        self.delete_button = ctk.CTkButton(self, text="Delete Product", bg_color="#FFFFFF", fg_color="#0C5481",
                                           text_color="#FFFCFC",
                                           border_color="#1572D3", width=159, height=44,
        ...
        self.print_qr_button = ctk.CTkButton(self, text="Print QR Code",
                                             font=("Inter", 16),
                                             width=159, height=44,
                                             bg_color="#FFFFFF", fg_color="#0C5481",
                                             text_color="#FFFCFC",
        ...
        self.print_qr_button.place(x=300 / 1920 * self.width, y=790 / 974 * self.height)

        # Treeview
        self.entries = {
            "Name": self.name_entry,
            "Category": self.category_combobox,
        ...
        style.configure("Treeview.Heading", font=("Inter", 18), rowheight=80, background="#0C5481", foreground="white")
        style.configure("Treeview", font=("Inter", 16), rowheight=32,background="#eaf9ff",
                        foreground="#057687",fieldbackground="#eaf9ff")
        frame = ctk.CTkFrame(self)
        frame.place(x=50 / 1920 * self.width, y=360 / 864 * self.height)
        self.product_treeview = ttk.Treeview(frame, columns=("Product ID", "Name", "Category", "Quantity",
                                                             "Price", "Status", "Register_Date"),
                                             show="headings")
        self.product_treeview.heading("Product ID", text="Product ID")
        self.product_treeview.column("Product ID", width=220, anchor="center")
        ...
        self.product_treeview.column("Quantity", width=150, anchor="center")

        self.product_treeview.heading("Price", text="Price(RM)")
        ...
        style.map("Treeview", background=[('selected', '#2A50CB')])
    ...
    def print_qr_code(self):
        ...
        try:
            ...
            # Convert CTkImage to PIL image
            qr_image = self.qr_img._light_image  # Extract underlying PIL image
            temp_img_path = "temp_qr_image.png"
            qr_image.save(temp_img_path)

            # Prepare PDF
            c = canvas.Canvas(file_path, pagesize=A4)
            width, height = A4

            # Insert QR image into PDF (centered)
            c.drawImage(temp_img_path, x=(width - 50 * mm) / 2, y=(height - 50 * mm) / 2, width=50 * mm, height=50 * mm)

            # Optional: Add product ID and name as labels
            selected = self.product_treeview.selection()
            if selected:
                values = self.product_treeview.item(selected, "values")
                c.setFont("Helvetica", 12)
                c.drawCentredString(width / 2, (height - 50 * mm) / 2 - 20, f"Product ID: {values[0]}")
                c.drawCentredString(width / 2, (height - 50 * mm) / 2 - 35, f"Name: {values[1]}")
            ...
            messagebox.showerror("Error", f"Failed to save QR code as PDF: {e}")
