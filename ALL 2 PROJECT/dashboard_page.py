...
class Dashboard:
    ...
    def add_to_cart(self):
        ...
        # Normalize item name to avoid mismatch due to spaces/case
        try:
            with sqlite3.connect('Trackwise.db') as conn:
                cursor = conn.cursor()
                cursor.execute("""
        ...
        # Update subtotal/tax/discount UI
    ...
    def handle_qr_scan(self, event):
        ...
        try:
            with sqlite3.connect('Trackwise.db') as conn:
                ...
                quantity_to_add = 1  # Default to 1 unit per scan
                ...
                # Optional: Show toast or beep
                print(f"✅ Item '{name}' added via QR scan.")
    ...
    def update_total_label(self):
        ...
        self.tax_amount = round(net_total * 0.06, 2)
        self.discount_amount = round((discount_percent / 100) * net_total, 2)

        # Final Total: Net + Tax - Discount
        self.final_total = round(net_total + self.tax_amount - self.discount_amount, 2)

        self.net_label.configure(text=f"Net: RM {net_total:.2f}")
        self.tax_label.configure(text=f"Tax: RM {self.tax_amount:.2f}")
